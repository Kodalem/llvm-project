...
PatmosMCCodeEmitter::getImmediateEncoding(const MCInst &MI, const MCOperand& MO,
  ...
  // Handle floating immediates using MCOperand APIs available in this LLVM.
  if (MO.isSFPImm()) {
    // Single-precision float encoded as a 32-bit pattern
    return static_cast<unsigned>(MO.getSFPImm());
  }
  if (MO.isDFPImm()) {
    // Double-precision immediate: use the high 32 bits of the 64-bit bitpattern
    uint64_t bits = MO.getDFPImm();
